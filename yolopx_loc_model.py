#!/usr/bin/env python3
"""
YOLOPX-Loc: Multi-Task Localization Model
...
Usage:
    from mtloc_model import create_yolopx_loc_model

    model = create_yolopx_loc_model(
...
# ---------------------------------------------------------------------------
# YOLOPX-Loc image encoder
# ---------------------------------------------------------------------------

class YOLOPXLocImageEncoder(nn.Module):
    """
...
# ---------------------------------------------------------------------------
# YOLOPXLocNet (main model)
# ---------------------------------------------------------------------------

class YOLOPXLocNet(BaseModel):
    """
    YOLOPX-Loc model: YOLOPX backbone + adapter + OrienterNet decoder.

    Inherits OrienterNet's architecture but replaces the image encoder
    with YOLOPXLocImageEncoder (frozen YOLOPX + lightweight adapter).
    """
    ...
    def _init(self, conf):
        self.image_encoder = YOLOPXLocImageEncoder(
...
def create_yolopx_loc_model(
    ...
    """
    Create YOLOPXLocNet with pretrained OrienterNet weights.

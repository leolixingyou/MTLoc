...
def load_yaml_config(yaml_path):
    """
    Load YAML configuration file and merge nested configs.

    Args:
        yaml_path: Path to main YAML configuration file
    ...
    # Load and merge DATA_CFG if specified
    if 'DATA_CFG' in config and 'PATH' in config['DATA_CFG']:
        data_cfg_path = config['DATA_CFG']['PATH']
        if os.path.exists(data_cfg_path):
            with open(data_cfg_path, 'r') as f:
                data_config = yaml.safe_load(f)
            # Merge DATASET configuration
            if 'DATASET' in data_config:
                if 'DATASET' not in config:
                    config['DATASET'] = {}
                config['DATASET'].update(data_config['DATASET'])

    # Load and merge MODEL_CFG if specified
    if 'MODEL_CFG' in config and 'PATH' in config['MODEL_CFG']:
        model_cfg_path = config['MODEL_CFG']['PATH']
        if os.path.exists(model_cfg_path):
            ...
            # Merge LOSS configuration
            if 'LOSS' in model_config:
                if 'LOSS' not in config:
                    config['LOSS'] = {}
                config['LOSS'].update(model_config['LOSS'])
...
def dict_to_namespace(d):
    """
    Recursively convert dict to namespace for dot notation access.

    Args:
        d: Dictionary or list to convert

    Returns:
        SimpleNamespace or list: Converted object
    """

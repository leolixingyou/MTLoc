...
class MultiHeadLoss(nn.Module):
    ...
    def _forward_impl(self, predictions, targets, shapes, model, imgs):
        ...
        # Handle training mode flags (方案B: Loss置零，保留梯度计算)
        if hasattr(cfg.TRAIN, 'DET_ONLY') and (cfg.TRAIN.DET_ONLY or (hasattr(cfg.TRAIN, 'ENC_DET_ONLY') and cfg.TRAIN.ENC_DET_ONLY)):
            da_seg_loss = 0 * da_seg_loss
            ll_seg_loss = 0 * ll_seg_loss
            ll_tversky_loss = 0 * ll_tversky_loss

        if hasattr(cfg.TRAIN, 'SEG_ONLY') and (cfg.TRAIN.SEG_ONLY or (hasattr(cfg.TRAIN, 'ENC_SEG_ONLY') and cfg.TRAIN.ENC_SEG_ONLY)):
            det_all_loss = 0 * det_all_loss
        ...
        # Return tensors for gradient analysis, convert to float only at logging time
        return loss, (det_all_loss, da_seg_loss, ll_seg_loss, ll_tversky_loss)

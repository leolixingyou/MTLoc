...
def build_targets(cfg, predictions, targets, model):
    ...
    for i in range(det.nl):
        ...
        if not isinstance(predictions, (list, tuple)):
            raise TypeError(f"predictions should be list/tuple, got {type(predictions)}")
        if i >= len(predictions):
            raise IndexError(f"predictions index {i} out of range, len={len(predictions)}")
        pred_i = predictions[i]
        if not hasattr(pred_i, 'shape'):
            raise AttributeError(f"predictions[{i}] has no shape attribute, type={type(pred_i)}")
        gain[2:6] = torch.tensor(pred_i.shape)[[3, 2, 3, 2]]  # xyxy gain

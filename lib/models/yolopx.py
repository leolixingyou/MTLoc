...
from lib.models.base_yolop import BaseYOLOP
...
# 修改
class MCnet(BaseYOLOP):
    # block_cfg = YOLOP-list
    def __init__(self, block_cfg, cfg=None, **kwargs):
        super(MCnet, self).__init__(cfg)
        self.block_cfg = block_cfg
        self.build_model(block_cfg)
        
    def build_model(self, block_cfg):
        """Build YOLOPX model architecture"""
        layers, save= [], []
        self.nc = 1
        ...
        # Feature visualization flags and storage are inherited from BaseYOLOP
        # self.visualize_features = False
        # self.feature_maps = {}
        self.feature_visualizations = {}  # Use the base class storage
        self.visualization_interval = 100  # Visualize every N batches
    ...
    def forward(self, x, input_image=None):
        ...
        # Store original input for visualization using base class method
        if self.visualize_features:
            self.stored_input_image = input_image if input_image is not None else x
            self.feature_visualizations.clear()  # Clear previous visualizations
        ...
        for i, block in enumerate(self.model):
            ...
            # Visualize features after specific blocks using base class method
            if self.visualize_features:
                # After backbone (ELANNet outputs at index 0)
                if i == 0:  # After backbone
                    if isinstance(x, (list, tuple)):
                        # Visualize C2, C3, C4, C5 features
                        if len(x) > 0 and x[0] is not None:
                            self.visualize_feature_map(x[0], 'backbone_c2')
                        if len(x) > 1 and x[1] is not None:
                            self.visualize_feature_map(x[1], 'backbone_c3')
                        if len(x) > 2 and x[2] is not None:
                            self.visualize_feature_map(x[2], 'backbone_c4')
                        if len(x) > 3 and x[3] is not None:
                            self.visualize_feature_map(x[3], 'backbone_c5')
                    else:
                        self.visualize_feature_map(x, 'backbone_output')
                # After PaFPNELAN (index 1)
                elif i == 1:  # After neck
                    if isinstance(x, (list, tuple)) and len(x) > 0:
                        # Visualize neck outputs for each branch
                        if len(x) > 6 and x[-1] is not None:
                            self.visualize_feature_map(x[-1], 'neck_to_det')
                        if len(x) > 2 and x[2] is not None:
                            self.visualize_feature_map(x[2], 'neck_to_seg1')
                        if len(x) > 3 and x[3] is not None:
                            self.visualize_feature_map(x[3], 'neck_to_seg2')
                # Just before detection head execution
                elif i == self.detector_index:
                    if not isinstance(x, (list, tuple)):
                        self.visualize_feature_map(x, 'det_head_input')
                # Just before segmentation heads
                elif i in self.seg_out_idx:
                    seg_idx = self.seg_out_idx.index(i)
                    self.visualize_feature_map(x, f'seg_head_{seg_idx}_output')
    ...
    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        ...
        for m in self.model.modules():
            ...
            #     m.switch_to_deploy()
                ...
                m.conv = self.fuse_conv_and_bn(m.conv, m.bn)  # update conv using base class method
    ...
    # Visualization methods are inherited from BaseYOLOP      

def get_net(cfg, **kwargs): 
    m_block_cfg = YOLOP
    model = MCnet(m_block_cfg, cfg=cfg, **kwargs)
    return model
